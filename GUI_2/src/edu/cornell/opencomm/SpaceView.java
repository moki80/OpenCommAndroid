...
public class SpaceView extends LinearLayout {
	...
	public void createIcons(Space space) { 
	  ...
	  public void addPerson(Person p) { 
		  PersonView pv = new PersonView(context, p, (int) (Math.floor(w *Math.random())), 
				  (int) (Math.floor(mainScreenH * Math.random())), 55, 55); 
		  this.icons.add(pv); 
	  }
	  ...
	  public boolean onTouchEvent(MotionEvent event) { 
		  ...
		  case MotionEvent.ACTION_UP: 
			  canDraw = PrivateSpaceView.currentSpaces.size();
			  /*If you highlited an icon, then clicked on nothing on screen,
			   it should unhighlite all the other icons */
			  ...
			  if (selectedIcon != null) { 
				  // if you did not move the icon and just clicked it, then highlite it
